...
export class BudgetResolverBase {
  ...
  @graphql.ResolveField(() => [BudgetItem], { name: "budgetItems" })
  async findBudgetItems(
    @graphql.Parent() parent: Budget,
    @graphql.Args() args: BudgetItemFindManyArgs
  ): Promise<BudgetItem[]> {
    const results = await this.service.findBudgetItems(parent.id, args);
  ...
  async getMove(@graphql.Parent() parent: Budget): Promise<Move | null> {
