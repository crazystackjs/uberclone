...
export class OrderResolverBase {
  ...
  async createOrder(@graphql.Args() args: CreateOrderArgs): Promise<Order> {
    return await this.service.createOrder({
      ...
      data: {
        ...
        deliveryAgent: args.data.deliveryAgent
          ? {
              connect: args.data.deliveryAgent,
            }
          : undefined,
  ...
  @graphql.ResolveField(() => DeliveryAgent, {
    nullable: true,
    name: "deliveryAgent",
  })
  async getDeliveryAgent(
    ...
    const result = await this.service.getDeliveryAgent(parent.id);
  ...
  async getVendor(@graphql.Parent() parent: Order): Promise<Vendor | null> {
  ...
  async PayOrder(
    @graphql.Args()
    args: PayOrderInput
  ): Promise<PayOrderOutput> {
    return this.service.PayOrder(args);
  }
}
